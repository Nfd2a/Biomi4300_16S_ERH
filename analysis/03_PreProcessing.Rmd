...
metadata_df <- 
  ...
  dplyr::select(.,run_accession, scientific_name, experiment_title) %>%
  filter(., grepl("Experiment 1",experiment_title)) %>%
  rename(c("sample_names" = "run_accession")) %>%
  mutate(soil_type = ifelse(grepl("Lansing", experiment_title),"Lansing","")) %>%
  mutate(soil_type = ifelse(grepl("Pendelton", experiment_title),"Pendelton",soil_type)) %>%
  mutate(soil_type = ifelse(grepl("Sand", experiment_title),"Sand",soil_type)) %>%
  mutate(soil_type = ifelse(grepl("Florence", experiment_title), "Florence", soil_type)) %>%
  mutate(soil_type = ifelse(grepl("fungus", scientific_name),"Neg",soil_type)) %>%
  mutate(soil_type = ifelse(grepl("synthetic", scientific_name),"Pos",soil_type)) %>% mutate(Sample_or_Control = ifelse(grepl("Neg", soil_type), FALSE, TRUE)) %>%
  mutate(Sample_or_Control = ifelse(grepl("Pos", soil_type), FALSE, Sample_or_Control))
...
<span style="color: red;">INTERPRETATION #1: There were 22 Chloroplasts and 155 Mitochondria in my sample, which is a lot, but compared to the 9614 total taxa, is a minor part of my data.</span> 
...
From the plot above, it actually looks like we should include `ASV_0004`, `ASV_0025`, `ASV_0043`, `ASV_0489`, and `ASV_0942` in our analyses. 
...
negControlASVs_rm <- dplyr::setdiff(neg_controlASVs_vec$ASV, c("ASV_0004", "ASV_0025", "ASV_0043", "ASV_0489", "ASV_0942"))
...
<span style="color: red;">INTERPRETATION #2: We removed 51 ASVs from negative controls. That seems reasonable since thats about 0.5% of the total ASV count. When looking at the graph, these ASVs also had comparatively low abundance in actual samples.</span>
...
That all 26 ASVs in the mocks were exact matches to the expected reference sequence

<span style="color: red;">INTERPRETATION #3: The dataset had multiple mock (positive control) communities. All the ASVs in the mocks matched to expected reference sequences so the sequencing run was accurate.</span>
...
<span style="color: red;">INTERPRETATION #4: For 0 ASVs, the reverse complement will need to be removed. There are no reverse complements in my dataset. </span> 
...
seqSums_df <- 
  ...
  left_join(., rownames_to_column(var = "sample_names", metadata_final_df), by = "sample_names")
...
numSeq_hist_plot <- 
  seqSums_df %>%
  ggplot(aes(x= TotalSeqs, fill = soil_type)) + 
...
numSeq_density_plot <- 
  seqSums_df %>%
  ggplot(aes(TotalSeqs, fill = soil_type)) +
...
<span style="color: red;">INTERPRETATION #5: I think the sequencing depth is ok. Most samples have over 10k reads. No samples should be removed at this point. </span>
