...
filtered_forward_reads <- 
  list.files(filtered_fastqs_path, pattern = "_1.fastq.gz",
             full.names = TRUE)
# Check 
...
filtered_reverse_reads <- 
    list.files(filtered_fastqs_path, pattern = "_2.fastq.gz",
             full.names = TRUE)
...
dada_forward$`SRR12183139_1.fastq.gz`
...
glimpse(merged_ASVs$`SRR12183139_1.fastq.gz`)
```
...
4. *What is the length of the trimmed ASVs after `filterAndTrim()`?* 
    - In our `01_QualityTrimming.Rmd`, we used `trimLeft = (19,20)` to remove the primers at the beginning of the forward read and `truncLen = c(225, 200)` to cut off base reads with declining quality.
    - Forward: `225 - 19 = 206`
    - Reverse: `200 - 20 = 180`
5. *What is the overlap between our forward and reverse reads?*
    ...
    - We had a total read length of 301 bp from multiQC, but excluding primers, the length should be 252 bp. This will be completely covered by the combined/overlapping length of the forward and reverse primers (100%).
...
In the table above, we can see that we actually have quite the ASV sequence length distribution! There are some longer ASVs that are clearly spurious... we need to be **skeptical**! We see that the most abundant ASV length is 253, which has order an of magnitude more ASVs than compared to any other length. Since the difference in ASV abundance at 253 is so much higher than any other ASV length, we will remove ASVs that are not 253 bp. 
...
As we saw in the table, the length of the ASVs is mostly 253 base pairs. 

<span style="color: red;">INTERPRETATION #2:  The ASV length data doesn't exactly match the hypotheses from INTERPRETATION #1. The expected length is 252 bp, while most reads are clustering at 253 bp (orders of magnitude larger than any other length). Whie not an exact match, it is pretty close. I'm not exactly where that extra base is coming from though. </span>


***

<span style="color: red;">INTERPRETATION #3: Taking into account INTERPRETATIONS #1 and #2, what is your suggested trimming procedure for your ASV lengths? Will you allow some variation or trim to an exact length, similar to our in class example here? For trimming, although there likely soome are spurious ASVs, there is a significant portion of reads that are one or two bases over. I'll be slightly more generous than the class example, and trim ASVs are that not in the 252-256 range. </span>

## Trim ASV lengths 

Now that we are pretty convinced that we have some spurious ASVs, we will trim ASVs that are not in the 252-256 range in this data set. 
...
*Let's trim the ASVs to only be the right size, which is 252-256!* 

```{r trim-ASVs}
# Subset only ASVs that are 245 bps long 
raw_ASV_table_trimmed <- 
  raw_ASV_table[,nchar(colnames(raw_ASV_table)) %in% seq(252, 256)]
...
<span style="color: red;">INTERPRETATION #4: (1) What proportion of raw ASVs were retained after trimming? 98.3% (2) What proportion of the trimmed ASVs were retained after chimera removal? 96.7% (3) What proportion of the total merged ASVs were retained after both trimming and chimera removal? 95%. I think the ASV lengths have been trimmed adequately, removing about 1/20 of the reads. Any more trimming would result in too much data being discarded. </span> 
...
- The distribution is roughly unimodal, centered around 20,000–40,000 reads per sample.
- Some samples have very low read counts (~0 reads)
- A couple of other samples have higher sequencing depth (>50,000 reads).

**Panel B: ASV Richness after DADA2:** Histogram of ASV richness (Total # of ASVs per sample)

- Trimodal distribution Most samples have ~500-1000 ASVs.
- Some samples have very low ASV counts (~0)
